...
export function AdminLoginForm() {
  ...
  const redirectTo = useMemo(() => searchParams.get("from") ?? "/admin", [searchParams]);

  const [adminId, setAdminId] = useState("admin12345");
  ...
  const handleSubmit = useCallback(
    ...
    [adminId, password, redirectTo, router]
  );

  const disableSubmit = status === "loading" || adminId.trim().length === 0 || password.length === 0;
    ...
    >
      <div className="relative h-full w-full rounded-[30px] bg-gradient-to-br from-white/35 via-white/25 to-white/10 px-10 py-12 dark:from-[#190707]/80 dark:via-[#140404]/70 dark:to-[#0b0202]/70">
        ...
        <motion.div className="mb-10 space-y-2" variants={floatingVariants} initial="hidden" animate="visible">
          ...
          <h1 className="text-4xl font-semibold text-foreground sm:text-5xl">Administrator Login</h1>
          <p className="text-sm text-muted-foreground sm:text-base">
            Use your Admin ID and password to access privileged controls. All actions are audited automatically.
          </p>
        </motion.div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <motion.div whileHover={{ scale: disableSubmit ? 1 : 1.01 }} whileTap={{ scale: disableSubmit ? 1 : 0.98 }}>
            ...
            >
              <span>{status === "loading" ? "Authenticating" : "Access Admin Panel"}</span>
